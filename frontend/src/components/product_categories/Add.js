...
const Add = ({ newAlert, removeAlert }) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const resp = await axios.post(`http://192.168.0.179:8000/api/product/categories`, categoriesData);
            ...
            if(!resp_data.success){    
                ...
                newAlert({
                    'id': alert_id,
                    'alert': <Alert id={alert_id} title={<><FontAwesomeIcon icon={faTimesCircle} /> Error!</>} body={resp_data.msg} color={'danger'} removeAlert={removeAlert} />
                });
                return;
            }
            ...
            newAlert({
                'id': alert_id,
                'alert': <Alert id={alert_id} title={<><FontAwesomeIcon icon={faCheckCircle} /> Exito!</>} body={resp_data.msg} color={'success'} removeAlert={removeAlert} />
            });
        } catch (error) {
            ...
            if (typeof resp_data.msg === 'string') {
                ...
                Object.values(resp_data.msg).forEach(errorMsg => {
                    ...
                    newAlert({
                        'id': alert_id,
                        'alert': <Alert id={alert_id} title={<><FontAwesomeIcon icon={faTimesCircle} /> Error!</>} body={errorMsg} color={'danger'} removeAlert={removeAlert} />
                    });
                });
            }
        }
